...
public class MergeSortGenerics<M extends Comparable<M>> {
    //merge sorting
    public static <M extends Comparable<M>> void mergeSorting(M[] array) {
        if (array == null) {
            return;             //checking array is null or not
        }
        if (array.length > 1) {
            Integer midIndex = array.length / 2;        //finding mid index

            //left half of the array
            String[] leftArray = new String[midIndex];           //left half = 0 -midindex
            for (Integer i = 0; i < midIndex; i++) {
                leftArray[i] = (String) array[i];
            }

            //right half of the array
            String[] rightArray = new String[array.length - midIndex];       //right half =midindex to (arraylength-mid)
            for (Integer j = midIndex; j < array.length; j++) {
                rightArray[j - midIndex] = (String) array[j];
            }

            mergeSorting(leftArray);          //recursive left half
            mergeSorting(rightArray);         // recursive right half

            Integer leftIndex = 0;     //left index value
            Integer rightIndex = 0;    //right index value
            Integer temp = 0;         //temp variable

            //merging two arrays right and left
            while (leftIndex < leftArray.length && rightIndex < rightArray.length) {
                if (leftArray[leftIndex].compareTo(rightArray[rightIndex]) < 0) {
                    array[temp] = (M) leftArray[leftIndex];
                    leftIndex++;
                } else {
                    array[rightIndex] = (M) rightArray[rightIndex];             //      merge sorting
            ...
            while (rightIndex < rightArray.length) {
                array[temp] = (M) rightArray[rightIndex];
                rightIndex++;
                temp++;
            }
        }
